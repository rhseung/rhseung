...
- **빌드·배포 명령은 `mise.toml` 의 `[tasks]` 에 있다.** 대시보드에는 `mise run cloudflare:build`
...
- 기간(`YearMonth`)은 `2026.08` 숫자 표기고 언어를 안 탄다. 글 날짜(dayjs `LL`)와 규약이
  다른 건 의도다 — `A - B` 쌍이 카드·이력서 폭에 걸리고 `tabular-nums` 정렬이 숫자라야 산다.
...
**PR 리뷰는 Claude 에 맡긴다** (`.github/workflows/review.yml`). push 마다 해결된 스레드를 닫고
새 변경분만 재검토하며, 남은 사항이 없으면 approve 한다. 코멘트에는 스레드 답글로 응답하고,
지시는 PR 코멘트에 `@claude` 멘션으로 한다 (`@claude review`).
