...
- **주석은 코드가 말할 수 없는 것만 적는다.** 무엇을 하는지는 코드가 이미 말한다 —
  적을 값어치가 있는 건 _왜 이 방식이어야 하는지_, 그것도 **다르게 짜면 코드가 깨지는
  이유**로 좁힌다 (Vite eager glob 의 TDZ, prose 가 덮어쓰는 마진, `useSyncExternalStore`의
  스냅숏 참조, 라이브러리·런타임·타입 시스템의 함정). **왜 이런 제품·UX·데이터 결정을
  내렸는지는 주석감이 아니다** — RSS 피드를 왜 하나로 뒀는지, 패딩을 왜 이 값으로 골랐는지
  같은 건 코드가 안 깨지는 선택지라 커밋 메시지나 문서에 남긴다. 함수·타입·prop 에 이름값을
  되풀이하는 설명형 JSDoc 을 붙이지 않는다.
