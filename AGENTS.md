...
**커밋 하나에 변경 하나.** 판정은 제목이 아니라 `git diff --staged` 로 한다 — 스테이지한
변경이 서로 독립적으로 되돌려질 수 있으면 이미 커밋 두 개다. 제목을 "A 하고 B 한다"로
쓰게 되는 건 그 결과일 뿐이고, 제목을 뭉뚱그린다고 커밋 하나가 되지 않는다.
리팩터와 기능, 서식과 로직은 같은 커밋에 넣지 않는다.

커밋하기 전에 항상 `git diff --staged --stat` 을 보고, 한 덩어리가 아니면 `git add -p`
또는 파일 단위로 나눠 담는다.

**제목은 영어, 본문은 한국어.** 제목은 `git log --oneline` 과 GitHub 목록에 그대로 서고
스크립트가 grep 하는 자리라 영어가 낫다. 영어로 옮기면 뜻이 흐려지는 고유명사와 도메인
용어만 한국어로 남긴다(`fix: 이력서 PDF 의 xref 오프셋을 지킨다`).
왜 그렇게 했는지, 무엇이 깨졌는지는 본문에 한국어로 길게 쓴다.

**문장부호는 ASCII 만** 쓴다. `·` `—` `→` `…` `“”` 같은 유니코드 기호는 터미널·훅·
이슈 트래커마다 폭이 달라 정렬이 깨지고, `git log --oneline` 을 grep 하기도 나쁘다.
쉼표와 하이픈으로 충분하다.
...
- **주석은 코드가 말할 수 없는 것만 적는다.** 무엇을 하는지는 코드가 이미 말한다 —
  적을 값어치가 있는 건 _왜 이 방식이어야 하는지_, 특히 **다르게 짜면 깨지는 이유**다
  (Vite eager glob 의 TDZ, prose 가 덮어쓰는 마진, `useSyncExternalStore` 의 스냅숏 참조).
