...
| 컬렉션     | 원본                                 | 언어 처리 |
| ---------- | ------------------------------------ | --------- |
| `posts`    | `content/posts/<slug>/<lang>.mdx`    | 파일 이름 |
| `projects` | `content/projects/<slug>/<lang>.mdx` | 파일 이름 |
...
![가로 사진 560x320](https://picsum.photos/seed/plain/560/320)

### 링크로 감싸기

제목을 달고 링크로 감싼 이미지.

[![같은 사진 560x320 을 링크로 감쌌다](https://picsum.photos/seed/plain/560/320 '마우스를 올리면 뜬다')](/ko/projects/)

### 캡션 달기

<Figure caption="캡션은 본문보다 작고 흐리게, 본문과 같은 축에 선다">

![캡션이 달린 가로 사진 560x320](https://picsum.photos/seed/plain/560/320)
...
<Callout tone="important" title="중요">

GitHub 의 알림 다섯 종을 그대로 따른다 - note, tip, important, warning, caution.
...
<Callout tone="caution" title="위험">

되돌릴 수 없는 일에만 쓴다. 다섯 개가 다 흔해지면 아무것도 안 눈에 띈다.
...
`src/content/posts/<slug>/` 에 `index.ts` 와 `ko.mdx` 를 두면 라우트가 생긴다.
번역이 없는 언어에서는 원문 위에 안내가 붙는다.
