...
| `LocalHostName` | Bonjour 이름. 같은 네트워크에서 `.local`로 접근할 때 | 일반 > 공유 > 로컬 호스트 이름 > 편집 |
| `HostName`      | 셸 프롬프트의 `@` 뒤                                 | 없음                                  |

앞의 둘은 GUI로 바꿀 수 있습니다. 다만 서로 다른 화면에 나뉘어 있어서, `정보`만 보면 `LocalHostName`은 안 바뀐 것처럼 보일 뿐입니다.

1. **시스템 설정 > 일반 > 정보 > 이름**에서 `ComputerName`을 바꿉니다.
2. **시스템 설정 > 일반 > 공유**로 들어가서, 하단 `로컬 호스트 이름` 옆의 **편집**을 눌러 `LocalHostName`을 바꿉니다. 공백도 한글도 못 받으니 영문 소문자와 하이픈으로 짓습니다.

`HostName`은 GUI 자체가 없습니다. 비워 두면 프롬프트에 `LocalHostName`이 `.local`을 달고 나오는데, 그 표기가 싫다면 터미널에서 직접 설정합니다.
