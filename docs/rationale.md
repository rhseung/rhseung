...
- **`AGENTS.md`가 규칙, 여기가 이유다.** 규칙은 세션마다 통째로 실리므로 짧게 두고, 겪은
  함정과 되돌린 시도는 여기 쌓는다. 규칙이 불편하면 끄기 전에 여기부터 읽고, 그래도 아니면
  물어봐라. 대부분은 이유가 있고, 없으면 같이 지우면 된다.
- 에이전트는 이 파일을 자동으로 읽지 않는다. `AGENTS.md`가 가리키거나 사람이 시킬 때 읽는다.
  그러니 **여기에만 있는 규칙을 만들지 않는다.** 규칙은 `AGENTS.md`에 한 줄로 올리고 근거만
  여기 둔다.
